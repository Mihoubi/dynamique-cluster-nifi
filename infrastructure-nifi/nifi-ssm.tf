...
# Nifi Registry
resource "aws_ssm_association" "tf-nifi-registry-ssm-assoc" {
  association_name = "${var.name_prefix}-registry-ssm-assoc-${random_string.tf-nifi-random.result}"
  name             = aws_ssm_document.tf-nifi-ssm-playbook-doc.name
  targets {
    key    = "tag:Name"
    values = ["registry.${var.name_prefix}${random_string.tf-nifi-random.result}.internal"]
  }
  output_location {
    s3_bucket_name = aws_s3_bucket.tf-nifi-bucket.id
    s3_key_prefix  = "ssm"
  ...
  parameters = {
    ExtraVariables = "SSM=True nifi_version=${var.nifi_version} registry_version=${var.registry_version} lb_dns=${aws_lb.tf-nifi-node-nlb.dns_name} s3_bucket=${aws_s3_bucket.tf-nifi-bucket.id} kms_key_id=${aws_kms_key.tf-nifi-kmscmk-s3.key_id} name_prefix=${var.name_prefix} name_suffix=${random_string.tf-nifi-random.result} web_port=${var.web_port} aws_region=${var.aws_region}"
    PlaybookFile   = "registry.yml"
    SourceInfo     = "{\"path\":\"https://s3.${var.aws_region}.amazonaws.com/${aws_s3_bucket.tf-nifi-bucket.id}/nifi/registry/\"}"
    SourceType     = "S3"
    Verbose        = "-v"
  }
  depends_on = [aws_iam_role_policy_attachment.tf-nifi-iam-attach-ssm, aws_iam_role_policy_attachment.tf-nifi-iam-attach-s3, aws_s3_object.tf-nifi-registry-files]
}
