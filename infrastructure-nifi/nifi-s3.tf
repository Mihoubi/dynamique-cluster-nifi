...
resource "aws_s3_bucket_acl" "tf-nifi-bucket" {
  ...
  acl    = "private"
  depends_on = [aws_s3_bucket_ownership_controls.tf-nifi-bucket]
}

# Resource to avoid error "AccessControlListNotSupported: The bucket does not allow ACLs"
resource "aws_s3_bucket_ownership_controls" "tf-nifi-bucket" {
  bucket = aws_s3_bucket.tf-nifi-bucket.id
  rule {
...
resource "aws_s3_object" "tf-nifi-registry-files" {
  for_each       = fileset("playbooks/registry/", "*")
  bucket         = aws_s3_bucket.tf-nifi-bucket.id
  key            = "nifi/registry/${each.value}"
  content_base64 = base64encode(file("${path.module}/playbooks/registry/${each.value}"))
  kms_key_id     = aws_kms_key.tf-nifi-kmscmk-s3.arn
}

resource "aws_s3_object" "tf-nifi-registry2-files" {
  bucket         = aws_s3_bucket.tf-nifi-bucket.id
  key            = "nifi/registry/nifi-registry-primary.mv.db"
  source = "/Users/mohamedmihoubi/Desktop/Formation_AWS/Formation_Nifi/infrastructure-nifi-up-to-date/playbooks/data-base/nifi-registry-primary.mv.db"
  kms_key_id     = aws_kms_key.tf-nifi-kmscmk-s3.arn
}